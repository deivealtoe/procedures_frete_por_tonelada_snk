...
BEGIN
	...
	-------------------------------------------------------------------------
	-- Objetivo: Calcular o Frete do pedido
	-------------------------------------------------------------------------
	...
	BEGIN
		...
		DECLARE
			...
			@VLRFRETECALCORIG	NUMERIC(12, 2), -- Não mais usado
		...
		BEGIN 
			SET @CODPARC = sankhya.EVP_GET_CAMPO_INT(@P_IDSESSAO, 'CODPARC') -- Não mais usado
			...
			BEGIN
				...
				BEGIN
					...
					FROM
						(
							SELECT
								NROUNICO,
								DTENTREGA,
								SUM(PESOBRUTOCALC) AS PESOTOTALITENS,
								CASE
									WHEN SUM(PESOBRUTOCALC) <= 1000 THEN 60
									WHEN SUM(PESOBRUTOCALC) > 1000 THEN ((SUM(PESOBRUTOCALC) - 1000) * 0.08) + 60
								END AS VLRFRETECALC
							FROM
								(
									SELECT
					...
					--SET @VLRDESC = ROUND(@VLRTOTITENS * @PERCDESC / 100,2)
					...
					FOR
						SELECT
							...
							/*ROUND((
								select
									cab_aux_desc_frete.ad_vlrdescfrete
								from
									tgfcab (nolock) cab_aux_desc_frete
								where
									cab_aux_desc_frete.nunota = nroUnico
							) / @COUNT, 2)*/
					...
					--IF @VLRFRETEATUAL > @VLRFRETE AND ISNULL(@VLRDESCFRETE,0) = 0
					--	SET @VLRFRETE = @VLRFRETEATUAL
					EXEC sankhya.EVP_SET_CAMPO_DEC @P_IDSESSAO, 'VLRFRETE', @VLRFRETE
		...
		BEGIN
			...
			BEGIN
				SET @VLRFRETEORIG = sankhya.EVP_GET_CAMPO_DEC(@P_IDSESSAO, 'AD_VLRFRETE')
				--SET @VLRFRETECALCORIG = sankhya.EVP_GET_CAMPO_DEC(@P_IDSESSAO, 'AD_VLRFRETE') 
				--SET @AD_VLRDESCFRETE = sankhya.EVP_GET_CAMPO_DEC(@P_IDSESSAO, 'AD_VLRDESCFRETE')
				SET @TIPFRETEORIG = sankhya.EVP_GET_CAMPO_TEXTO(@P_IDSESSAO, 'AD_TIPFRETE')
				SET @CIF_FOBORIG  = sankhya.EVP_GET_CAMPO_TEXTO(@P_IDSESSAO, 'AD_CIF_FOB')
				SET @CODPARCTRANSPORIG = sankhya.EVP_GET_CAMPO_INT(@P_IDSESSAO, 'AD_CODPARCTRANSP')
				...
				EXEC sankhya.EVP_SET_CAMPO_DEC @P_IDSESSAO, 'VLRFRETE', @VLRFRETEORIG
				--EXEC sankhya.EVP_SET_CAMPO_DEC @P_IDSESSAO, 'VLRFRETECALC', @VLRFRETECALCORIG
